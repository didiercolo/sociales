export const lessonsData = {
    4: [
        // 4th grade lessons go here
    ],
    5: [
        {
            id: 1,
            title: "Lección 1: Costa Rica y su Geografía",
            description: "Costa Rica y su Geografía",
            sections: [
                {
                    title: "1. Líneas Imaginarias",
                    content: [
                        "Las líneas imaginarias nos ayudan a ubicar lugares en la Tierra. No son reales, pero son muy importantes.",
                        "<h3>Paralelos</h3><ul><li>Van de este a oeste.</li><li>Miden la <strong>latitud</strong>.</li><li>El más importante es el <strong>Ecuador (0°)</strong>.</li><li>Divide la Tierra en Hemisferio Norte y Hemisferio Sur.</li></ul>",
                        "<h3>Meridianos</h3><ul><li>Van de norte a sur.</li><li>Miden la <strong>longitud</strong>.</li><li>El más importante es el <strong>Meridiano de Greenwich (0°)</strong>.</li><li>Divide la Tierra en Hemisferio Oriental y Hemisferio Occidental.</li></ul>"
                    ],
                    videoId: "r6Mg005jCds"
                },
                {
                    title: "2. Ubicación de Costa Rica",
                    content: [
                        "Costa Rica se ubica en:",
                        "<ul><li><strong>Hemisferio Norte</strong> (al norte del Ecuador).</li><li><strong>Hemisferio Occidental</strong> (al oeste de Greenwich).</li><li>En <strong>América Central</strong>, entre América del Norte y América del Sur.</li><li>Entre el Océano Pacífico y el Mar Caribe.</li></ul>"
                    ]
                },
                {
                    title: "3. Océanos y Continentes",
                    content: [
                        "<h3>Océanos</h3><p>Los cinco océanos son: Pacífico, Atlántico, Índico, Ártico y Antártico.</p>",
                        "<h3>Continentes</h3><p>Los continentes son: Asia, África, Europa, América, Oceanía y Antártida.</p>"
                    ],
                    videoId: "QDfJ4m1t1mQ"
                },
                {
                    title: "4. Costas de Costa Rica",
                    content: [
                        "<h3>Costa Pacífica</h3><ul><li>Es más grande.</li><li>Tiene muchos golfos, bahías y penínsulas.</li><li>Importante para turismo y pesca.</li></ul>",
                        "<h3>Costa Caribe</h3><ul><li>Es más pequeña.</li><li>Tiene playas hermosas.</li><li>Importante para puertos y comercio.</li></ul>"
                    ]
                },
                {
                    title: "5. Sistemas Montañosos",
                    content: [
                        "Principales cordilleras:<ul><li>Cordillera Volcánica Central</li><li>Cordillera de Talamanca</li><li>Cordillera de Guanacaste</li><li>Cordillera de Tilarán</li></ul>",
                        "Estas montañas tienen volcanes, ayudan a formar ríos, afectan el clima y son importantes para la naturaleza y el turismo."
                    ]
                },
                {
                    title: "6. Ventajas y Desventajas de la Ubicación",
                    content: [
                        "<h3>Ventajas</h3><ul><li>Acceso a dos océanos.</li><li>Rica en biodiversidad.</li><li>Bellas playas y bosques.</li><li>Buenas oportunidades de comercio.</li></ul>",
                        "<h3>Desventajas</h3><ul><li>Puede tener terremotos y huracanes.</li><li>Inundaciones en algunas áreas.</li><li>Actividad volcánica.</li></ul>"
                    ]
                },
                {
                    title: "7. Importancia de Estudios Sociales y Cívica",
                    content: [
                        "<p><strong>Estudios Sociales:</strong> Nos ayuda a entender historia y geografía, aprender sobre nuestro país y ser ciudadanos responsables.</p>",
                        "<p><strong>Educación Cívica:</strong> Nos enseña respeto, responsabilidad, democracia y cómo vivir en paz con los demás.</p>"
                    ]
                }
            ],
            quiz: [
                {
                    question: "¿Qué nos ayudan a ubicar las líneas imaginarias?",
                    options: ["Lugares en la Tierra", "Estrellas en el cielo", "Peces en el mar"],
                    correct: 0
                },
                {
                    question: "¿Cómo se llaman las líneas que van de este a oeste?",
                    options: ["Meridianos", "Paralelos", "Ecuador"],
                    correct: 1
                },
                {
                    question: "¿Cuál es el paralelo más importante?",
                    options: ["Meridiano de Greenwich", "El Ecuador", "Trópico de Cáncer"],
                    correct: 1
                },
                {
                    question: "¿Qué miden los paralelos?",
                    options: ["Longitud", "Latitud", "Altitud"],
                    correct: 1
                },
                {
                    question: "¿En qué hemisferios divide el Ecuador a la Tierra?",
                    options: ["Norte y Sur", "Este y Oeste", "Arriba y Abajo"],
                    correct: 0
                },
                {
                    question: "¿Cómo se llaman las líneas que van de norte a sur?",
                    options: ["Paralelos", "Meridianos", "Ecuador"],
                    correct: 1
                },
                {
                    question: "¿Cuál es el meridiano más importante?",
                    options: ["Ecuador", "Greenwich", "Capricornio"],
                    correct: 1
                },
                {
                    question: "¿Qué miden los meridianos?",
                    options: ["Latitud", "Longitud", "Temperatura"],
                    correct: 1
                },
                {
                    question: "¿En qué hemisferio está Costa Rica con respecto al Ecuador?",
                    options: ["Sur", "Norte", "Este"],
                    correct: 1
                },
                {
                    question: "¿En qué hemisferio está Costa Rica con respecto a Greenwich?",
                    options: ["Oriental", "Occidental", "Norte"],
                    correct: 1
                },
                {
                    question: "¿Entre qué continentes está Costa Rica?",
                    options: ["Europa y Asia", "América del Norte y del Sur", "África y Oceanía"],
                    correct: 1
                },
                {
                    question: "¿Cuáles son los dos océanos que bañan a Costa Rica?",
                    options: ["Atlántico e Índico", "Pacífico y Caribe (Atlántico)", "Ártico y Antártico"],
                    correct: 1
                },
                {
                    question: "¿Cuál es la costa más grande de Costa Rica?",
                    options: ["Atlántica", "Pacífica", "Son iguales"],
                    correct: 1
                },
                {
                    question: "¿Cuál costa tiene muchos golfos y bahías?",
                    options: ["Caribe", "Pacífica", "Ninguna"],
                    correct: 1
                },
                {
                    question: "¿Cuál de estas es una ventaja de la ubicación de Costa Rica?",
                    options: ["Muchos desiertos", "Rica biodiversidad", "Mucha nieve"],
                    correct: 1
                },
                {
                    question: "¿Cuál es una desventaja natural de Costa Rica?",
                    options: ["Puede tener terremotos", "No tiene playas", "No hay montañas"],
                    correct: 0
                },
                {
                    question: "¿Qué cordillera NO está en Costa Rica?",
                    options: ["Talamanca", "Guanacaste", "Los Andes"],
                    correct: 2
                },
                {
                    question: "¿Qué nos enseña la Educación Cívica?",
                    options: ["Matemáticas", "Respeto y Democracia", "Inglés"],
                    correct: 1
                },
                {
                    question: "¿Cuántos océanos existen en total?",
                    options: ["3", "5", "7"],
                    correct: 1
                },
                {
                    question: "¿Cuál continente habitamos?",
                    options: ["Europa", "América", "Asia"],
                    correct: 1
                }
            ]
        }
    ],
    6: [
        {
            id: 1,
            title: "Lección 1: La Campaña Nacional de Costa Rica (1856-1857)",
            description: "La Campaña Nacional de Costa Rica",
            sections: [
                {
                    title: "Conociendo a los personajes",
                    content: [
                        "<p><strong>Juan Rafael Mora Porras:</strong> Nació en San José en 1814. Fue presidente de Costa Rica tres veces (1849-1859). Promovió el café, el alumbrado público y dirigió la Campaña Nacional.</p>",
                        "<p><strong>William Walker:</strong> Estadounidense que intentó conquistar Centroamérica. Se declaró presidente de Nicaragua en 1856.</p>"
                    ]
                },
                {
                    title: "Antecedentes de la Campaña",
                    content: [
                        "<p>Fue una serie de batallas (1856-1857) donde Costa Rica defendió Centroamérica de los filibusteros.</p>",
                        "<h3>Imperialismo norteamericano</h3><p>Estados Unidos creía en el 'destino manifiesto' y la 'Doctrina Monroe', buscando expandirse sobre América.</p>"
                    ]
                },
                {
                    title: "Situación de Nicaragua",
                    content: [
                        "<p>Nicaragua tenía una guerra interna entre Liberales y Conservadores. Walker aprovechó esto para tomar el poder.</p>",
                        "<h3>Vía del Tránsito</h3><p>Nicaragua era clave para viajar a California por el río San Juan y el lago.</p>"
                    ]
                },
                {
                    title: "Llegaron los filibusteros",
                    content: [
                        "<p><strong>¿Qué es un filibustero?</strong> Invasores extranjeros que usan la fuerza para apoderarse de territorios.</p>",
                        "<p>Mora se enteró del peligro por sus diplomáticos y preparó al pueblo con la proclama del 20 de noviembre de 1855.</p>"
                    ]
                }
            ],
            quiz: [
                {
                    question: "¿Quién dirigió el ejército de Costa Rica en la Campaña Nacional?",
                    options: ["William Walker", "Juan Rafael Mora Porras", "Felipe Molina"],
                    correct: 1
                },
                {
                    question: "¿En qué años se llevó a cabo la Campaña Nacional?",
                    options: ["1821-1823", "1856-1857", "1948-1949"],
                    correct: 1
                },
                {
                    question: "¿Qué doctrina decía 'América para los americanos'?",
                    options: ["Doctrina Monroe", "Destino Manifiesto", "Tratado Cañas-Jerez"],
                    correct: 0
                },
                {
                    question: "¿Dónde nació William Walker?",
                    options: ["San José, Costa Rica", "Nashville, Tennessee", "León, Nicaragua"],
                    correct: 1
                },
                {
                    question: "¿Qué grupo contrataron los liberales en Nicaragua?",
                    options: ["Mercenarios/Filibusteros", "Cofetaleros", "Ingleses"],
                    correct: 0
                },
                {
                    question: "¿Qué ruta estratégica buscaban controlar en Nicaragua?",
                    options: ["Vía del Tránsito", "Carretera Interamericana", "Ferrocarril al Atlántico"],
                    correct: 0
                },
                {
                    question: "¿Cómo se enteró Mora del peligro de Walker?",
                    options: ["Por internet", "Por sus diplomáticos en Washington", "Por los periódicos de Nicaragua"],
                    correct: 1
                },
                {
                    question: "¿Qué fecha tuvo la primera proclama de Mora?",
                    options: ["11 de abril de 1856", "20 de noviembre de 1855", "1 de mayo de 1857"],
                    correct: 1
                },
                {
                    question: "¿Cuál era la principal profesión de William Walker?",
                    options: ["Solo militar", "Médico, abogado y periodista", "Comerciante de café"],
                    correct: 1
                },
                {
                    question: "¿Qué país era la principal potencia imperialista en el siglo XIX?",
                    options: ["España", "Inglaterra", "Francia"],
                    correct: 1
                },
                {
                    question: "¿Cuántas veces fue presidente Mora Porras?",
                    options: ["1", "3", "5"],
                    correct: 1
                },
                {
                    question: "¿Qué recurso usaban los filibusteros para comprar armas?",
                    options: ["Oro de California", "Recursos de estados del sur de EE.UU.", "Impuestos de Nicaragua"],
                    correct: 1
                },
                {
                    question: "¿Qué grupo nicaragüense contactó a Walker?",
                    options: ["Conservadores", "Liberales", "Socialistas"],
                    correct: 1
                },
                {
                    question: "¿Qué ciudad nicaragüense era el centro de los conservadores?",
                    options: ["León", "Granada", "Managua"],
                    correct: 1
                },
                {
                    question: "¿Qué ciudad nicaragüense era el centro de los liberales?",
                    options: ["Granada", "León", "Rivas"],
                    correct: 1
                },
                {
                    question: "¿Qué se construyó durante el gobierno de Mora?",
                    options: ["El Teatro Nacional", "La Fábrica Nacional de Licores", "El Aeropuerto Juan Santamaría"],
                    correct: 1
                },
                {
                    question: "¿Qué sentimiento promovió Mora ante la guerra?",
                    options: ["Miedo", "Unidad y amor por la patria", "Indiferencia"],
                    correct: 1
                },
                {
                    question: "¿Qué país intentó Pierce comprarle a España?",
                    options: ["México", "Cuba", "Puerto Rico"],
                    correct: 1
                },
                {
                    question: "¿Quién era el presidente de EE.UU. en ese tiempo?",
                    options: ["Abraham Lincoln", "Franklin Pierce", "George Washington"],
                    correct: 1
                },
                {
                    question: "¿Qué tipo de armas compraron los filibusteros?",
                    options: ["Cañones y rifles", "Espadas y lanzas", "Tanques de guerra"],
                    correct: 0
                }
            ]
        }
    ]
};
